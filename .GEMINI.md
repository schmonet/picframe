...
* update `change.log`, `README.md` only on User Request
...
* `picframe/DEVELOPMENT.md` Technical documentation of development history, challenges, and decisions.
* `picframe/TESTING.md` Detailed results of video playback and image loading performance tests.

### Picframe Source Code 🛠️
...
### System Scripts, Installer and Service-Setup 🛠️
...
* `picframe/scripts/pir_manager.py` checks PIR sensor for movement, controls display via CEC and slideshow via HTTP, supports night mode override.
* `picframe/scripts/sync_photos.sh` manages unified cache for photos and videos, handling separate quotas and downloading randomly folder-wise from SMB share.
* `picframe/scripts/set_tty_color.sh` sets the TTY console background to black to hide boot messages/login prompt on the HDMI output.

### Testing Scripts and content 🛠️
...
* `picframe/test/videos` extended set of multi purpose test videos to test playback capabilities with different resolutions, aspect ratio, codecs, bitrates including color test patterns
* `picframe/test/images` extended set of multi purpose test images to test Ken Burns and display capabilities with different resolutions, aspect ratio including color test patterns
* `picframe/test/test_image_video_player.py` script to test both video and image playback in an alternating sequence
* `picframe/test/test_bulk_video_player.py` comprehensive test script to systematically evaluate video playback performance using `mpv` and `ffprobe`.
* `picframe/test/test_video_slide_show.py` tests the frame extraction and slideshow playback logic for videos using `ffmpeg` and `pi3d`.
...
* Extension of the Ken Burns effect, if `kenburns: true`
    ...
    * **Panorama:** If *image width > screen width*
        * Horizontal scrolling (Left/Right) instead of just zoom.
        * `kenburns_panorama_scroll_direction`: `left`, `right`, or `random`.
        * `kenburns_panorama_zoom_pct`: Zoom percentage applied during panorama scrolling.
        * `kenburns_panorama_zoom_direction`: `in`, `out`, or `random`.
        * `panorama_crop_to_aspect_ratio`: Optional cropping for super-wide panoramas.
